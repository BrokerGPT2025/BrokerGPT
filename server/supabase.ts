...
// The DNS fix module will have already fixed these environment variables if needed
...
async function connectToSupabase(attempt = 1): Promise<boolean> {
  ...
  try {
    ...
    // Use the URL directly - DNS fix module will have already fixed it if needed
    console.log(`Using Supabase URL: ${supabaseUrl.replace(/pnik.*\.co/, '<redacted>')}`);
